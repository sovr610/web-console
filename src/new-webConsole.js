class WebConsole {
    constructor(containerId = 'web-console') {
        ...
        this.setupStyles();
        this.tabs = ['Default', 'Event Logger', 'Elements','Network'];
    ...
    getFormattedSourceCode() {
        ...
        return this.formatHTML(sourceCode);
    }
    ...
    formatHTML(html) {
        ...
        const lines = html.split(/>\s*</).filter(Boolean);
        ...
        lines.forEach((line, index) => {
            ...
            formatted += tab.repeat(indent) + '<' + currentLine + '>\n';

            if (currentLine.match(/^<?\w[^>]*[^\/]$/) && 
                !currentLine.startsWith('input') &&
                !currentLine.startsWith('img') &&
                !currentLine.startsWith('br') &&
                !currentLine.startsWith('hr') &&
                !currentLine.startsWith('meta')) {
                indent++;
            }
        });
        ...
        const highlightHTML = (html) => {
            return html.replace(/(&lt;\/?)(\w+)([\s\S]*?)(&gt;)/g, (match, p1, p2, p3, p4) => {
                return `<span class="html-tag">${p1}${p2}</span>${highlightAttributes(p3)}${p4}`;
            });
        };

        const highlightAttributes = (attrs) => {
            return attrs.replace(/(\s+)(\w+)(=)(&quot;|&#39;)(.*?)(\4)/g, (match, space, attr, equals, quote, value) => {
                return `${space}<span class="html-attr">${attr}</span>${equals}<span class="html-attr-value">${quote}${value}${quote}</span>`;
            });
        };

        const highlightCSS = (css) => {
            return css.replace(/([^{\}]+)(\{[^}]*\})/g, (match, selector, rules) => {
                const highlightedSelector = selector.replace(/([^\s,]+)/g, '<span class="css-selector">$1</span>');
                const highlightedRules = rules.replace(/([^:]+):([^;]+);/g, '<span class="css-property">$1</span>:<span class="css-value">$2</span>;');
                return `${highlightedSelector}${highlightedRules}`;
            });
        };

        const highlightJS = (js) => {
            const keywords = ['var', 'let', 'const', 'function', 'return', 'if', 'else', 'for', 'while', 'class', 'new', 'try', 'catch', 'throw', 'async', 'await'];
            let highlighted = js;

            // Highlight keywords
            highlighted = highlighted.replace(new RegExp(`\\b(${keywords.join('|')})\\b`, 'g'), '<span class="js-keyword">$1</span>');

            // Highlight strings
            highlighted = highlighted.replace(/((&quot;|&#39;).*?\2)/g, '<span class="js-string">$1</span>');

            // Highlight numbers
            highlighted = highlighted.replace(/\b(\d+(?:\.\d+)?)\b/g, '<span class="js-number">$1</span>');

            // Highlight function calls
            highlighted = highlighted.replace(/(\w+)(?=\s*\()/g, '<span class="js-function">$1</span>');
        ...
        const highlightedLines = lines.map(line => {
            ...
            else if (line.includes('&lt;style')) inStyle = true;
            else if (line.includes('&lt;/style')) inStyle = false;

            if (inScript) return highlightJS(highlightHTML(line));
            if (inStyle) return highlightCSS(highlightHTML(line));
    ...
    setupElementsTab() {
        const elementsContent = this.consoles['Elements'];
        const sourceCode = this.getFormattedSourceCode();
        const pre = document.createElement('pre');
        pre.innerHTML = this.highlightCode(sourceCode);
        pre.style.margin = '0';
        ...
        `;
        elementsContent.appendChild(style);
    ...
    interceptNetworkRequests() {
        ...
        window.fetch = function(...args) {
            ...
            return originalFetch.apply(this, args).then(response => {
                ...
                self.logNetworkRequest(args[0], 'fetch', response.status, response.headers.get('Content-Type'), '-', duration);
                return response;
            });
        };
        ...
        XMLHttpRequest.prototype.send = function() {
            ...
            this.addEventListener('load', function() {
                ...
                this.console.logNetworkRequest(self._url, self._method, this.status, contentType, size, duration);
            });
            originalXHRSend.apply(this, arguments);
        };
    }

    logNetworkRequest(url, method, status, type, size, time) {
        const request = { url, method, status, type, size, time };
        this.networkRequests.push(request);
        this.updateNetworkTab(request);
    }

    updateNetworkTab(request) {
    ...
    setupNetworkTab() {
        const networkContent = this.consoles['Network'];
        const table = document.createElement('table');
        table.style.width = '100%';
        table.style.borderCollapse = 'collapse';
        table.innerHTML = `
        ...
        `;
        networkContent.appendChild(table);
    }
}